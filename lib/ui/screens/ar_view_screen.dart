...
class _ArViewScreenState extends State<ArViewScreen> with SingleTickerProviderStateMixin {
  ...
  void _initSensors() {
    ...
    _sensorSub = _sensorsService.orientationStream.listen((data) {
      ...
      _sensorsService.triggerAlignmentHaptic(dev);
    });
  }
  ...
  Widget build(BuildContext context) {
    final double elevationTilt = GeoCalculator.calculateElevationTilt(
      widget.tower.distanceMeters,
      0.0,
      kDefaultTowerHeight,
    );
        ...
        children: [
          ...
          RepaintBoundary(
            child: AnimatedBuilder(
              animation: Listenable.merge([_sensorsService.headingNotifier, _sensorsService.pitchNotifier]),
              builder: (context, _) {
                final heading = _sensorsService.headingNotifier.value;
                final pitch = _sensorsService.pitchNotifier.value;
                final dev = GeoCalculator.calculateAngularDeviation(heading, widget.tower.azimuthBearing);
                final pitchDiff = pitch - elevationTilt;
                final bool isAligned = dev.abs() <= MovistarConstants.defaultAlignedToleranceDegrees &&
                    pitchDiff.abs() <= MovistarConstants.defaultAlignedToleranceDegrees;
          ...
          RepaintBoundary(
            child: AnimatedBuilder(
              animation: Listenable.merge([
                _sensorsService.headingNotifier,
                _sensorsService.pitchNotifier,
                _pulseController,
              ]),
              builder: (context, _) {
                final heading = _sensorsService.headingNotifier.value;
                final pitch = _sensorsService.pitchNotifier.value;
                final deviation = GeoCalculator.calculateAngularDeviation(heading, widget.tower.azimuthBearing);
                final pitchDiff = pitch - elevationTilt;
                final bool isAligned = deviation.abs() <= MovistarConstants.defaultAlignedToleranceDegrees &&
                    pitchDiff.abs() <= MovistarConstants.defaultAlignedToleranceDegrees;
                ...
                return LayoutBuilder(
                  builder: (context, constraints) {
                    ...
                    // When device is tilted up (pitchDiff > 0), target moves down on camera screen
            ...
            child: Align(
              ...
              child: AnimatedBuilder(
                ...
                builder: (context, _) {
                  ...
                  final dev = GeoCalculator.calculateAngularDeviation(heading, widget.tower.azimuthBearing);
                  final pitchDiff = pitch - elevationTilt;
                  final bool isHeadingAligned = dev.abs() <= MovistarConstants.defaultAlignedToleranceDegrees;
                  final bool isPitchAligned = pitchDiff.abs() <= MovistarConstants.defaultAlignedToleranceDegrees;
